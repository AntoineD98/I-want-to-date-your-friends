...
class Network:
    ...
    def get_direct_connections(self, session: Session, user_id: str) -> Set[str]:
    ...
    def get_discovery_network(self, user_id: str) -> Optional[Dict[str, List[str]]]:
        with Session(self.engine) as session:
            connections = self.get_direct_connections(session, user_id)
            if not connections:
                return {}
            ...
            for connection_id in connections:
                connection_connections = self.get_direct_connections(session, connection_id)
                ...
                for connection_connection_id in connection_connections:
                    if connection_connection_id != user_id and connection_connection_id not in connections:
                        connection_connection = session.get(User, connection_connection_id)
